...
from mlgf.lib.helpers import get_custom_freq_gfhf_features, get_hyb_off, gGW_mo_saiao, get_sigma_fit, exclude_core_ftrs, get_orbtypes, get_g0
from mlgf.lib.helpers import get_chk_saiao, get_sigma_fit, get_core_orbital_indices, get_orb_type, get_orb_principal, get_orbtypes_df, get_saiao_charges, get_saiao_locality
from mlgf.workflow.ml_features import get_saiao_features
...
class MoldatumPickler(pickle.Pickler):
...
class Moldatum:
    ...
    def save(self, f):
        if hasattr(f, 'write'):
            MoldatumPickler(f, protocol=5).dump(self.data)
        else:
            with open(f, 'wb') as fp:
                MoldatumPickler(fp, protocol=5).dump(self.data)
    ...
    @staticmethod
    def load_chk(f, purge_keys = [], force_eigv_direction = True, val_core_dats = None, symmetrize_sigmaI = True):
        """primary load function for Moldatum objects (from chk files)
        ...
        Returns:
            Moldatum: the resulting Moldatum for the molecule, with SAIAO features in numpy format
        """        
        ...
        mol = pyscf.lib.chkfile.load_mol(f)

        if not val_core_dats is None:
        ...
        mf.xc = mlf.get('xc', 'pbe0').decode('utf-8')
        scf_data = pyscf.lib.chkfile.load(f, 'scf')
        ...
        md = Moldatum({key: value for key, value in mlf.items() if key not in purge_keys})
        if 'mol' in md.data:
            if isinstance(md.data['mol'], str):
                    md.data['mol'] = Mole.loads(md.data['mol'])
        md.fname = f
    ...
    def load(f):
        ...
        if hasattr(f, 'read'):
            return Moldatum(pickle.load(f))
        else:
            # assume f is a path-like object
            with open(f, 'rb') as fp:
                return Moldatum(pickle.load(fp))
    ...
    def calc_dyn(self, dyn_imag_freq_points, ftr_suffix = '', add_ef = True):
        ...
        gf_dyn, hyb_dyn = get_custom_freq_gfhf_features(
            self.mo_energy, self.fock_saiao, self.C_saiao_mo, dyn_imag_freq_points, mlf_chkfile = getattr(self, 'fname', ''))
        setattr(self, f'gf_dyn{ftr_suffix}', gf_dyn)
        setattr(self, f'hyb_dyn{ftr_suffix}', hyb_dyn)

        hyb_dyn_off = get_hyb_off(self.fock_saiao, gf_dyn, dyn_imag_freq_points)
    ...
    def get_diag_features(self, features, ret_labels=False, exclude_core = False):
        ...
        if exclude_core: inds_core = self.inds_core
        to_stack = []
        for f in features:
            if 'hyb_dyn' in f:
                feat = self.data[f]
