...
from fcdmft.gw.mol.gw_ac import _get_ac_idx, _get_scaled_legendre_roots
...
def get_sigma_ml(mlf_chkfile, pickle_file):
    ...
    """    
    if  '.joblib' in model_file: model = joblib.load(pickle_file)
    sigma = model.predict_full_sigma(mlf_chkfile)
    return sigma

def get_vmo(scf_chkfile, xc = 'hf'):
    """get MO basis vk and vxc

    Args:
        scf_chkfile (str): DFT calculation file
        xc (str, optional): dft functional. Defaults to 'hf'.

    Returns:
        np.float64, np.float64: vk_mo, vxc_mo
    """    
    mlf = pyscf.lib.chkfile.load(scf_chkfile, 'mlf')
...
def get_pyscf_input_mol(mf, auxbasis=None, nocc_act=None, nvir_act=None):
    ...
    max_memory = max(2000, (mf.max_memory-mem_now)*0.3)
    blksize = int(min(naux, max(mf.with_df.blockdim, (max_memory*1e6/8)/(nmo*nmo))))
    Lpq = []
    for eri1 in mf.with_df.loop(blksize=blksize):
        eri_3d = None
        eri_3d = _ao2mo.nr_e2(eri1, mo, ijslice, aosym='s2', mosym='s1', out=eri_3d)
        Lpq.append(eri_3d)
    del eri1
    del eri_3d
...
def get_hyb_off(fock_saiao, g0_saiao, omega):
...
#     print(stacks.shape)
    for w in range(nomega):
        g0_inv = np.linalg.pinv(stacks[:,w,:,:])
        hyb_off_saiao[:, w] = omega_minus_fock[:,w] - g0_inv[:, 0, 1]
    
    result = np.zeros(g0_saiao.shape, dtype = np.complex128)
    result[iu] = hyb_off_saiao
    result[il] = result.transpose((1, 0, 2))[il]
    # assert(np.linalg.norm(result[:,:,0] - result[:,:,0].T) < 1e-8)
...
def get_chk_saao(mol, custom_chkfile):  
    """only has symmetrization, no IAO

    Args:
        mol (pyscf.mol):
        custom_chkfile (str): mlgf/pyscf chkfile

    Returns:
        float64: AO to SAAO rotation
    """      
    hf = pyscf.lib.chkfile.load(custom_chkfile, 'mlf')
    dm = hf['dm_hf']
    C_ao_saao = get_saao(mol, dm)
    return C_ao_saao
