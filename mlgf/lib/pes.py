...
try:
    ...
    warnings.warn(f"PES fitting is not available: {e}")
...
def hybridization_fitting(Delta, Z, mmax=6, maxiter=50, disp=True):
    ...
    if len(Delta.shape) == 1:
        r0 = aaa_real(Delta, Z, mmax=mmax)
    else:
        r0 = aaa_matrix_real(Delta, Z, mmax=mmax)
    pol = np.real(r0.pol())
    weight, _, residue = get_weight(pol, Z, Delta, cleanflag=True)
    pol, weight = aaa_reduce(pol, weight, 1e-5)

    def fhere(pole): return erroreval(pole, Z, Delta, cleanflag=True)
    res = scipy.optimize.minimize(fhere, pol, method='L-BFGS-B', jac=True,
                                  options={"disp": False, "maxiter": 4000, "gtol": 1e-15, "ftol": 1e-15})

    def fhere(pole): return erroreval(pole, Z, Delta, cleanflag=False)
    res = scipy.optimize.minimize(
        fhere, res.x, method='L-BFGS-B', jac=True,
        options={"disp": disp, "maxiter": maxiter, "gtol": 1e-20, "ftol": 1e-20})
    weight, _, residue = get_weight(res.x, Z, Delta, cleanflag=False)
